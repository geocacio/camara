@extends('panel.index')
@section('pageTitle', 'Cartas de Serviços')
...
<div class="card">
    <div class="card-body">
        ...
        <div class="card-header text-right header-with-search">
            ...
            ['routerSearch' => 'serviceLetter.index',
            'clearRouterSearch' => ['route' => 'serviceLetter.index', 'params' => ['search' => '', 'perPage' => $perPage]]])
            ...
            <div class="btn-group dropleft">
                ...
                <div class="dropdown-menu">
                    <a class="dropdown-item" href="{{ route('serviceLetter.indexPage') }}">Página</a>
                    <a class="dropdown-item" href="{{ route('serviceLetter.create') }}">Novo</a>
                    <a class="dropdown-item" href="{{ route('subcategories.index', 'cartas-de-servicos') }}">Categorais</a>
                </div>
            </div>
            ...
            <table class="table table-striped">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>{{ $service->title }}</td>
                        <td>
                        @if(isset($service->files[0]) && isset($service->files[0]->file) && isset($service->files[0]->file->url))
                            <a href="{{ asset('storage/'.$service->files[0]->file->url) }}" target="_blank">{{ $service->files[0]->file->url }}</a>
                            @endif
                        </td>
                        <td class="actions">
                            <a href="{{ route('serviceLetter.edit', $service->slug) }}" class="link edit"><i class="fas fa-edit"></i></a>
                                ...
                                <div class="modal-dialog modal-dialog-centered">
                                    ...
                                    <div class="modal-content">
                                        ...
                                        <div class="modal-footer">
                                            ...
                                            <form id="delete-form-{{ $service->id }}" action="{{ route('serviceLetter.destroy', $service->slug) }}" method="post" style="display: none;">
                                                @csrf
                                                @method('DELETE')
                                            </form>
                                        </div>
        ...
        @else
        <div class="no-data">
            <span>Ainda não existem Cartas de Serviços cadastrados.</span>
        </div>
        @endif

    </div>
    @include('panel.partials.footerCardWithPagination', ['routePerPage' => 'serviceLetter.index', 'paginate' => $serviceLetter])
</div>
@endsection
