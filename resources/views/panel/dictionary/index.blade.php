...
<div class="card">
    <div class="card-body">
        ...
        <div class="card-header text-right header-with-search">
            <a href="{{ route('dictionary.create') }}" class="btn-default">Novo</a>
        ...
        @if($dicionaries && $dicionaries->count() > 0)
        <div class="table-responsive">
            <table class="table table-striped">
                ...
                <tbody>
                    @foreach($dicionaries as $dictionary)
                    <tr>
                        <td>{{ $dictionary->id }}</td>
                        <td>{{ $dictionary->title }}</td>
                        <td>{{ $dictionary->description }}</td>
                        <td class="actions text-center">
                            <a href="{{ route('dictionary.edit', $dictionary->slug) }}" class="link edit"><i class="fas fa-edit"></i></a>
                        
                            <a data-toggle="modal" data-target="#myModal-{{ $dictionary->id}}" class="link delete"><i class="fas fa-trash-alt"></i></a>
                        
                            <div id="myModal-{{$dictionary->id}}" class="modal fade modal-warning" role="dialog">
                                    ...
                                    <div class="modal-content">
                                        ...
                                        <div class="modal-footer">
                                            ...
                                            <a href="#" class="btn btn-default" onclick="event.preventDefault();
                                                document.getElementById('delete-form-{{ $dictionary->id }}').submit();">
                                                Deletar
                                            </a>

                                            <form id="delete-form-{{ $dictionary->id }}" action="{{ route('dictionary.destroy', $dictionary->slug) }}" method="post" style="display: none;">
                                                @csrf
                                                @method('DELETE')
                                            </form>
                                        </div>
