...
<div class="card">
    <div class="card-body">
        ...
        <div class="card-header text-right">
            <a href="{{ route('constructions.progress.create', $construction->slug) }}" class="btn-default">Novo</a>
        </div>


        @if($construction->generalProgress && $construction->generalProgress->count() > 0)
        <div class="table-responsive">
            <table class="table table-striped">
                ...
                <tbody>
                    @foreach($construction->generalProgress as $progress)
                    <tr>
                        <td>{{ $progress->id }}</td>
                        <td>{{ $progress->date }}</td>
                        <td>{{ $progress->situation }}</td>
                        <td class="actions text-center">
                            <a href="{{ route('constructions.progress.edit', ['construction' => $construction->slug, 'progress' => $progress->id]) }}" class="link edit"><i class="fas fa-edit"></i></a>
                                ...
                                <div class="modal-dialog modal-dialog-centered">
                                    ...
                                    <div class="modal-content">
                                        ...
                                        <div class="modal-footer">
                                            ...
                                            <form id="delete-form-{{ $progress->id }}" action="{{ route('constructions.progress.destroy', ['construction' => $construction->slug, 'progress' => $progress->id]) }}" method="post" style="display: none;">
                                                @csrf
                                                @method('DELETE')
                                            </form>
                                        </div>
