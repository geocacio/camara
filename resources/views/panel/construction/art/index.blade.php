    ...
    <div class="card-body">
        ...
        <div class="card-header text-right">
            <div class="btn-group dropleft">
                ...
                <div class="dropdown-menu">
                    <a class="dropdown-item" href="{{ route('art.create', $construction->slug) }}">Novo</a>
                    <a class="dropdown-item" href="{{ route('subtypes.index', 'constructions-art') }}">Tipos</a>
                </div>
            </div>
        </div>


        @if($construction->arts && $construction->arts->count() > 0)
        <div class="table-responsive">
            <table class="table table-striped">
                ...
                <tbody>
                    @foreach($construction->arts as $art)
                    <tr>
                        <td>{{ $art->id }}</td>
                        <td>{{ $art->number }}</td>
                        <td>{{ date('d/m/Y', strtotime($art->date)) }}</td>
                        <td>{{ $art->responsible }}</td>
                        <td class="actions text-center">
                            <a href="{{ route('art.edit', ['construction' => $construction->slug, 'art' => $art->slug]) }}" class="link edit"><i class="fas fa-edit"></i></a>
                            <a data-toggle="modal" data-target="#myModal-{{ $art->id }}" class="link delete"><i class="fas fa-trash-alt"></i></a>

                            <div id="myModal-{{ $art->id }}" class="modal fade modal-warning" role="dialog">
                                <div class="modal-dialog modal-dialog-centered">
                                    ...
                                    <div class="modal-content">
                                        ...
                                        <div class="modal-footer">
                                            ...
                                            <form id="delete-form-{{ $art->id }}" action="{{ route('art.destroy', ['construction' => $construction->slug, 'art' => $art->slug]) }}" method="post" style="display: none;">
                                                @csrf
                                                @method('DELETE')
                                            </form>
                                        </div>
        ...
        @else
        <div class="no-data">
            <span>Ainda não existem art cadastradas.</span>
        </div>
        @endif
