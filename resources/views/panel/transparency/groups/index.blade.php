@extends('panel.index')
@section('pageTitle', 'Grupos de organização para o Portal da Transparência')
...
<div class="card">
    <div class="card-body">
        ...
        <div class="card-header text-right">
            <a href="{{ route('transparency.groups.create') }}" class="btn-default">Novo</a>
        </div>

        @if($transparencyGroup && $transparencyGroup->count() > 0)
        <div class="table-responsive">
            <table class="table table-striped">
                ...
                <tbody>
                    @foreach($transparencyGroup as $group)
                    <tr>
                        <td>{{ $group->id }}</td>
                        <td>{{ $group->title }}</td>
                        <td>{{ Str::limit($group->description, 71, '...') }}</td>
                        <td class="actions">
                            <a href="{{ route('transparency.groups.edit', $group->slug) }}" class="link edit"><i class="fas fa-edit"></i></a>
                            <a data-toggle="modal" data-target="#myModal{{$group->id}}" class="link delete"><i class="fas fa-trash-alt"></i></a>

                            <div id="myModal{{$group->id}}" class="modal fade modal-warning" role="dialog">
                                <div class="modal-dialog modal-dialog-centered">
                                    ...
                                    <div class="modal-content">
                                        ...
                                        <div class="modal-footer">
                                            ...
                                            <form id="delete-form-{{ $group->id }}" action="{{ route('transparency.groups.destroy', $group->slug) }}" method="post" style="display: none;">
                                                @csrf
                                                @method('DELETE')
                                            </form>
                                        </div>
