@extends('panel.index')
@section('pageTitle', 'Serviços externos')
...
<div class="card">
    <div class="card-body">
        ...
        ['withSearch' => true,
        'routerSearch' => 'external-links.index',
        'clearRouterSearch' => ['route' => 'external-links.index', 'params' => ['search' => '', 'perPage' => $perPage]],
        'routerNew' => 'external-links.create'])

        @if($externalLink && $externalLink->count() > 0)
            ...
            <table class="table table-striped">
                ...
                <tbody>
                    ...
                    <tr>
                        <td>{{ $link->id }}</td>
                        <td>{{ $link->title }}</td>
                        <td>{{ $link->visibility == 'enabled' ? 'Ativado' : 'Desativado' }}</td>
                        <td class="actions">
                            <a href="{{ route('external-links.edit', $link->slug) }}" class="link edit"><i class="fas fa-edit"></i></a>
                                ...
                                <div class="modal-dialog modal-dialog-centered">
                                    ...
                                    <div class="modal-content">
                                        ...
                                        <div class="modal-footer">
                                            ...
                                            <form id="delete-form-{{ $link->id }}" action="{{ route('external-links.destroy', $link->slug) }}" method="post" style="display: none;">
                                                @csrf
                                                @method('DELETE')
                                            </form>
                                        </div>
    ...
    @include('panel.partials.footerCardWithPagination', ['routePerPage' => 'external-links.index', 'paginate' => $externalLink])
</div>
@endsection
