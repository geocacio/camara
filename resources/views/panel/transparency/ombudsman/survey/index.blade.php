...
<div class="card">
    <div class="card-body">
        ...
        <div class="card-header text-right">
            <div class="btn-group container-dropdown-menu">
                <button type="button" class="btn btn-secondary dropdown-toggle btn-dropdown" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                    <i class="fa-solid fa-bars"></i>
                </button>
                <div class="dropdown-menu dropdown-menu-right">
                    <ul class="menu">
                        <li class="item">
                            <a href="{{ route('ombudsman.survey.page') }}" class="link">Página</a>
                        </li>
                        <li class="item">
                            <a href="{{ route('ombudsman-survey-questions.index') }}" class="link">Perguntas</a>
                        </li>
                    </ul>
                </div>
            </div>
        ...
        @if($ombudsman_survey && $ombudsman_survey->count() > 0)
        <div class="table-responsive">
            <table class="table table-striped">
                ...
                <tbody>
                    @foreach($ombudsman_survey as $survey)
                    <tr>
                        <td>{{ $survey->id }}</td>
                        <td>{{ $survey->cpf }}</td>
                        <td class="actions text-right">
                            <a data-toggle="modal" data-target="#showSurvey{{$survey->id}}" class="link see"><i class="fa-solid fa-eye"></i></a>
                            <!-- Permitir somente o admin apagar a manifestação -->
                            @if($user->id == 1)
                            <a data-toggle="modal" data-target="#myModal{{$survey->id}}" class="link delete"><i class="fas fa-trash-alt"></i></a>
                            <div id="myModal{{$survey->id}}" class="modal fade modal-warning" role="dialog">
                                <div class="modal-dialog modal-dialog-centered">
                                    ...
                                    <div class="modal-content">
                                        ...
                                        <div class="modal-footer">
                                            ...
                                            <form id="delete-form-{{ $survey->id }}" action="{{ route('ombudsman-survey.destroy', $survey->id) }}" method="post" style="display: none;">
                                                @csrf
                                                @method('DELETE')
                                            </form>
                                        </div>
                            ...
                            <div id="showSurvey{{$survey->id}}" class="modal fade" role="dialog">
                                <div class="modal-dialog modal-dialog-centered">
                                    <!-- Modal content-->
                                    <div class="modal-content">
                                        <div class="modal-body">
                                            <div class="container-satisfation-survey-response">
                                                <h3 class="title text-center">{{ $survey->cpf }}</h3>
                                                <div class="row">
                                                    @foreach($survey->responses as $response)
                                                    <div class="col-12">
                                                        <div class="item">
                                                            <span class="label">{{ $response->question->question_text }}</span>
                                                            <span class="value">
                                                                @if($response->note == 1)
        ...
        @else
        <div class="no-data">
            <span>Ainda não existem Pesquisa de satisfação cadastradas.</span>
        </div>
        @endif
