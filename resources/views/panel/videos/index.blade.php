...
<div class="card">
    <div class="card-body">
        ...
        <div class="card-header text-right">
            <a href="{{ route('videos.create') }}" class="btn-default">Novo</a>
        </div>
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>
                            ...
                            @foreach($video->files as $thumbnail)
                            @if(in_array(pathinfo($thumbnail->file->url, PATHINFO_EXTENSION), ['png', 'jpg', 'jpeg', 'gif', 'webp']))
                            <img class="image-table" src="{{ asset('storage/'.$thumbnail->file->url) }}" />
                            @endif
                            @endforeach
                            @endif
                        </td>
                        <td>{{ $video->title }}</td>
                        <td class="actions">
                            <a href="{{ route('videos.edit', $video->slug) }}" class="link edit"><i class="fas fa-edit"></i></a>
                            <a data-toggle="modal" data-target="#myModal{{$video->id}}" class="link delete"><i class="fas fa-trash-alt"></i></a>

                            <div id="myModal{{$video->id}}" class="modal fade modal-warning" role="dialog">
                                <div class="modal-dialog modal-dialog-centered">
                                    ...
                                    <div class="modal-content">
                                        ...
                                        <div class="modal-footer">
                                            ...
                                            <form id="delete-form-{{ $video->id }}" action="{{ route('videos.destroy', $video->slug) }}" method="post" style="display: none;">
                                                @csrf
                                                @method('DELETE')
                                            </form>
                                        </div>
        ...
        @else
        <div class="no-data">
            <span>Ainda não existem vídeos cadastrados.</span>
        </div>
        @endif
