...
<div class="card">
    <div class="card-body">
        ...
        <div class="card-header text-right">
            <a href="{{ route('employees.create') }}" class="btn-default">Novo</a>
        </div>
            ...
            <table class="table table-striped">
                ...
                <tbody>
                    ...
                    <tr>
                        <td>{{ $employee->id }}</td>
                        <td>
                            @if($employee->files && $employee->files->count() > 0)
                            <img class="image-table" src="{{ asset('storage/'.$employee->files[0]->file->url) }}" />
                            @endif
                        </td>
                        <td>{{ $employee->name }}</td>
                        <td class="actions">
                            <a href="{{ route('employees.edit', $employee->slug) }}" class="link edit"><i class="fas fa-edit"></i></a>
                            <a data-toggle="modal" data-target="#myModal{{$employee->id}}" class="link delete"><i class="fas fa-trash-alt"></i></a>

                            <div id="myModal{{$employee->id}}" class="modal fade modal-warning" role="dialog">
                                <div class="modal-dialog modal-dialog-centered">
                                    ...
                                    <div class="modal-content">
                                        ...
                                        <div class="modal-footer">
                                            ...
                                            <form id="delete-form-{{ $employee->id }}" action="{{ route('employees.destroy', $employee->slug) }}" method="post" style="display: none;">
                                                @csrf
                                                @method('DELETE')
                                            </form>
                                        </div>
