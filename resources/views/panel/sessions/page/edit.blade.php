...
<div class="card">
    <div class="card-body">
        <form action="{{ route('sessions.page.update', $page_session->id) }}" method="post" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            <div class="row">
                <div class="col-md-2">
                    <div class="form-group">
                        <label>Ícone</label>
                        <input type="text" name="icon" class="form-control icon" autocomplete="off" value="{{ old('icon', $page_session->icon) }}" onfocus="getIconInputValues(event)">
                    </div>
                </div>
                <div class="col-md-5">
                    <div class="form-group">
                        <label>Título Principal</label>
                        <input type="text" name="main_title" class="form-control" autocomplete="off" value="{{ old('main_title', $page_session->main_title) }}">
                    </div>
                </div>
                <div class="col-md-5">
                    <div class="form-group">
                        <label>Título</label>
                        <input type="text" name="title" class="form-control" autocomplete="off" value="{{ old('title', $page_session->title) }}">
                    </div>
                </div>
            </div>
            ...
            <div class="form-group">
                ...
                <select name="transparency_group_id" class="form-control">
                    ...
                    <option value="{{ $group->id }}" {{ $page_session->groupContents && $page_session->groupContents->transparency_group_id && $group->id == $page_session->groupContents->transparency_group_id ? 'selected' : '' }}>{{ $group->title }} - {{ $group->description }}</option>
                    @endforeach
                </select>
            </div>
            ...
            <div class="form-group">
                ...
                <div class="d-flex align-items-center justify-content-center w-fit-content actions">
                    <div class="toggle-switch cmt-4">
                        <input type="checkbox" id="checklist" name="visibility" value="enabled" class="toggle-input" {{ $page_session->visibility == 'enabled' ? 'checked' : ''}}>
